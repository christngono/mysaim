...
export default function UserDashboard({ onGoLanding }) {
  const { user, logout, refreshEnrollments, enrollments } = useAuth()
  const { lang }         = useLang()
  const t                = useT(lang)
  ...
  const DashHeader = () => (
    <header className="sticky top-0 z-[60] bg-white border-b border-slate-200 shadow-sm">
      <div className="flex items-center h-14 px-4 gap-3">
        ...
        {/* ─ Badge Premium ou bouton Débloquer ─ */}
        <div className="ml-auto flex items-center gap-2 flex-shrink-0">
          {enrollments.some(e => e.status === 'paid') ? (
            <span className="hidden sm:flex items-center gap-1.5 bg-amber-400 text-white text-xs font-extrabold px-3 py-1.5 rounded-full shadow-sm">
              ⭐ Accès Premium
            </span>
          ) : (
            <button
              onClick={() => setPaymentFormation(formations.find(f => f.id === activeFormationId) || formations[0] || null)}
              className="hidden sm:flex items-center gap-1.5 bg-saim-600 hover:bg-saim-700 text-white text-xs font-extrabold px-3 py-1.5 rounded-full shadow-sm transition-all active:scale-95"
            >
              🔓 Débloquez vos cours
            </button>
          )}
